...
@section('breadcrumb', 'Rekap pengeluaran barang')

@section('content')

    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('owner.laporan.keluar') }}" class="row g-2 align-items-end">
                <div class="col-sm-4">
                ...
                <div class="col-sm-4 d-flex gap-2">
                    ...
                    <a href="{{ route('owner.laporan.keluar') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center py-3">
            <span class="fw-semibold">
                <i class="bi bi-box-arrow-up text-danger me-2"></i>Daftar Barang Keluar
            </span>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($barangKeluars as $item)
                            <tr>
                                <td class="ps-3 text-muted small">{{ $barangKeluars->firstItem() + $loop->index }}</td>
                                <td class="fw-medium">{{ $item->nomor_keluar }}</td>
        ...
        @if ($barangKeluars->hasPages())
            <div class="card-footer bg-white border-top d-flex justify-content-between align-items-center py-3">
                <small class="text-muted">
                    Menampilkan {{ $barangKeluars->firstItem() }}–{{ $barangKeluars->lastItem() }}
                    dari {{ $barangKeluars->total() }} data
                </small>
                {{ $barangKeluars->withQueryString()->links('pagination::bootstrap-5') }}
            </div>
        @endif
    </div>
